...
export default function App() {
  ...
  const startCall = async () => {
    const chat = io('http://192.168.0.102:8000/chat');
    chat.on('error', err => console.log(err));
    console.log('Joined Room');
    chat.emit('room-join', 'Test');
    chat.on('this', data => console.log(data));
    ...
    localPC.onicecandidate = e => {
      try {
        ...
        if (e.candidate) {
          ...
          chat.emit('OnIceCandidate', e.candidate);
        }
      } catch (err) {
    ...
    chat.on('OnIceCandidate', IceCandidate => {
